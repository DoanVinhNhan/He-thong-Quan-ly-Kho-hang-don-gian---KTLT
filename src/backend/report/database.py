...
def db_get_revenue_data(start_date_str, end_date_str, group_by='day'):
    """Lấy dữ liệu doanh thu xuất kho theo thời gian."""
    conn = get_db_connection()
    params = []
...
def db_get_dashboard_stats():
    """Lấy các thống kê chính cho trang chủ."""
    conn = get_db_connection()
    cursor = conn.cursor()
    stats = {
        'total_products': 0,
    ...
    try:
        ...
        # 4. Giá trị kho hiện tại (Tổng số lượng tồn * đơn giá của mỗi sản phẩm)
        ...
        # 5. Tổng doanh thu từ trước đến nay (Tổng tiền của các giao dịch xuất)
